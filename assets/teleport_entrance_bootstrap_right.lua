function on_state_changed(state)
  if state == GameState.STATE_LEVEL_START then
    ...
    local label = entity:spawn('teleport_entrance_turquoise_right', spatial.x, spatial.y - 8, spatial.rotation)
...
function on_teleport_entrance_hit(source, target)
  ...
  if this.pair_id == source then
    renderer:play_animation('teleport_entrance_closed_right')
  end
end
